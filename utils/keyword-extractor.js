...
exports.mapVoiceToSkill = mapVoiceToSkill;
exports.mapVoiceToExperience = mapVoiceToExperience;
exports.mapVoiceToPay = mapVoiceToPay;
const constants_1 = require("./constants");
function extractKeywords(englishText) {
    const lower = englishText.toLowerCase();
    const found = new Set();
    for (const skill of constants_1.SKILL_LIST) {
    ...
    for (const cuisine of constants_1.CUISINE_LIST) {
...
function mapVoiceToSkill(englishText) {
    const lower = englishText.toLowerCase();
    for (const skill of constants_1.SKILL_LIST) {
        for (const kw of skill.keywords) {
            if (lower.includes(kw)) {
...
function mapVoiceToExperience(englishText) {
    const lower = englishText.toLowerCase();
    // Hindi number words
    const hindiNumbers = {
...
function mapVoiceToPay(englishText) {
    const lower = englishText.toLowerCase();
