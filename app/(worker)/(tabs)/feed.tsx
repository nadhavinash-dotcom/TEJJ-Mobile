...
async function resolveLocation(): Promise<UserCoords> {
...
async function fetchFeedPage(
  ...
  if (relocation || radiusKm === null) {
    params.set('max_distance_km', '20000');
  } else if (radiusKm) {
    params.set('max_distance_km', String(radiusKm));
  }
  const res = await api.get(`/jobs/feed?${params}`);
  return res.data as { success: boolean; data: any[]; page: number; has_more: boolean };
}
        ...
        <View className="absolute bottom-4 left-4 right-4 flex-row justify-between items-end">
          ...
          {job.distance_km !== undefined && (
            <View className="bg-white/90 backdrop-blur-md px-3 py-1.5 rounded-full flex-row items-center gap-1.5 shadow-sm">
              ...
              <Text className="font-bold text-[11px] text-slate-900">{job.distance_km} km</Text>
            </View>
          )}
        </View>
      </View>
        ...
        <View className="flex-row flex-wrap gap-2 mb-6">
          {job.pay_rate && (
            <View className={`flex-row items-center gap-1.5 px-3 py-2 rounded-xl ${isAboveMarket ? 'bg-emerald-50 border border-emerald-100' : 'bg-slate-50'}`}>
              ...
              <Text className={`text-sm font-bold ${isAboveMarket ? 'text-emerald-700' : 'text-slate-700'}`}>
                ₹{job.pay_rate}{' '}
                <Text className="font-medium text-xs">/ {job.pay_type === 'PER_SHIFT' ? 'Shift' : 'Month'}</Text>
              </Text>
            </View>
          )}
          {job.shift_duration_hours && (
            <View className="bg-slate-50 flex-row items-center gap-1.5 px-3 py-2 rounded-xl">
              ...
              <Text className="text-sm font-medium text-slate-700">{job.shift_duration_hours}h Shift</Text>
            </View>
          )}
          {job.number_of_openings && (
            <View className="bg-slate-50 flex-row items-center gap-1.5 px-3 py-2 rounded-xl">
              <Briefcase color="#475569" size={16} />
              <Text className="text-sm font-medium text-slate-700">{job.number_of_openings} Openings</Text>
            </View>
          )}
        </View>
...
export default function WorkerFeedScreen() {
  ...
  const [coords, setCoords] = useState<UserCoords>(undefined as any);

  // Resolve location once on mount
  React.useEffect(() => {
    resolveLocation().then(setCoords);
  }, []);

  const locationResolved = coords !== undefined;
  ...
  const {
    ...
    queryKey: ['feed', activeRadius, relocation, coords],
    queryFn: ({ pageParam }) => fetchFeedPage(pageParam as number, coords, radiusKm, relocation),
    initialPageParam: 1,
      ...
      <View className="bg-white p-5 rounded-2xl mb-8 shadow-sm border border-slate-100 space-y-5">
        <View>
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <TouchableOpacity
                onPress={() => setActiveRadius(item)}
                className={`px-5 py-2.5 rounded-xl border mr-2 ${activeRadius === item ? 'bg-primary border-primary' : 'bg-white border-slate-200'}`}
              >
                <Text className={`font-semibold text-[13px] ${activeRadius === item ? 'text-white' : 'text-slate-600'}`}>
                  {item}
                </Text>
              </TouchableOpacity>
            )}
        ...
        <View className="mt-4 flex-row items-center justify-between bg-slate-50 p-4 rounded-xl border border-slate-100">
          ...
          <TouchableOpacity
            onPress={() => setRelocation(r => !r)}
            className={`w-12 h-6 rounded-full justify-center px-1 ${relocation ? 'bg-primary' : 'bg-slate-300'}`}
          >
  ...
  const ListFooter = (
    <View>
      ...
      <View className="bg-primary rounded-2xl p-8 relative overflow-hidden mt-4 mb-4">
        <View className="absolute -right-16 -top-16 w-48 h-48 bg-on-primary-fixed-variant rounded-full blur-2xl" />
        <Text className="font-bold text-2xl text-white mb-3 z-10">Can't find the right fit?</Text>
  ...
  return (
    <SafeScreen bgClassName="bg-slate-50">
      <FlatList
        data={jobs}
